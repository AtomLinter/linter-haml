...
export default {
  activate() {
    ...
    this.subscriptions.add(
      atom.config.observe('linter-haml.hamlLintExecutablePath', (value) => {
  ...
  provideLinter() {
    return {
      ...
      lint: (textEditor) => {
        ...
        const hamlLintYmlPath = helpers.find(options.cwd, '.haml-lint.yml');
        if (hamlLintYmlPath) {
        ...
        return helpers.exec(executablePath, parameters, options).then((output) => {
          if (textEditor.getText() !== text) {
            const warnMessage = 'File contents changed since the lint was triggered';
            const warnOptions = {
              description: 'The file was modified since the request was sent ' +
                'to `linter-haml` to check it. Since any results would no longer ' +
                'be valid, they are not being updated. Please save the file again ' +
